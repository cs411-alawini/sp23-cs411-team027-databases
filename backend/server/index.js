...
app.get('/adv1',(req,res)=>{
    var query = connection.query('SELECT DISTINCT Price, COUNT(GameName) FROM GameData NATURAL JOIN GenreData WHERE Price < 10 AND (Multiplayer = "TRUE" OR Action = "TRUE") GROUP BY Price UNION SELECT DISTINCT Price, COUNT(GameName) FROM GameData NATURAL JOIN GenreData WHERE Price > 30 AND SinglePlayer = "TRUE" GROUP BY Price ORDER BY Price DESC LIMIT 15', function (err, rows, fields) {
res.send(rows)
console.log(rows)
console.log(err)   
 })
...
app.get('/adv2',(req,res)=> {
   var query = connection.query('SELECT Rating, RequiredAge, COUNT(GameName) FROM	GameData WHERE GameName IN (SELECT GameName FROM GameData NATURAL JOIN GenreData WHERE Multiplayer = "TRUE" AND Action = "TRUE") GROUP BY Rating, RequiredAge ORDER BY Rating DESC, RequiredAge DESC LIMIT 15', function (err, rows, fields) {
  res.send(rows)
}) 
})
